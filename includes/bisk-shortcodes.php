...
final class BISKShortCodes {
	...
	public function season_opening( $atts ) {
		...
		if ( '' !== $opening_date ) {
			...
			$todayobj     = current_datetime();
			$todaystr     = $todayobj->format( 'Y-m-d H-i-s' );
	...
	public function cost( $atts ) {
		...
		if ( '' !== $atts['item'] ) {
			$sql      = "SELECT m.meta_value FROM {$wpdb->prefix}posts p INNER JOIN {$wpdb->prefix}postmeta m ON p.ID=m.post_id WHERE post_type='bisk-costs' AND meta_key='bisk-costs-amount' AND REPLACE(LOWER(post_title), ' ', '_')=%s";
